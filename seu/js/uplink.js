...
function getDeviceUplink(par1, par2)
{
	...
	if(XMLHttpRequestObjectUplink)
	{
		if(!par2)
		{
			XMLHttpRequestObjectUplink.open("POST", 'ajax/getDeviceUplink.php?dev_id=' + par1, false);
			XMLHttpRequestObjectUplink.send(null);
			przetwarzajUplink(null);
		}
		else
		{
			XMLHttpRequestObjectUplink.open("POST", 'ajax/getDeviceUplink.php?parent_id=' + par1+'&local_dev_model=' + par2, false);
			XMLHttpRequestObjectUplink.send(null);
			przetwarzajUplink(null);
		}
	}
}
function getHostUplink(par1, par2)
{
	...
	if(XMLHttpRequestObjectUplink)
	{
		...
		{
			XMLHttpRequestObjectUplink.open("POST", 'ajax/getDeviceUplink.php?host_parent=' + par1+'&host_id=' + par2, false);
			XMLHttpRequestObjectUplink.send(null);
			przetwarzajUplink(null);
		}
...
function getHostUplinkWithPort(par1, par2)
{
	...
	if(XMLHttpRequestObjectUplink)
	{
		...
		else
		{
			XMLHttpRequestObjectUplink.open("POST", 'ajax/getDeviceUplink.php?host_parent=' + par1, false);
			XMLHttpRequestObjectUplink.send(null);
			przetwarzajUplink(par2);
		}

	}
}
function przetwarzajUplink(port)
{
	...
	if(XMLHttpRequestObjectUplink)
	{
		if(XMLHttpRequestObjectUplink.readyState == 4 && XMLHttpRequestObjectUplink.status == 200)
		{
			...
			{	
				wygenerujUplink(lista, port);
			}
		}
	}
}
function wygenerujUplink(lista, port)
{
	//div = uplinks
	...
	else
	{
		...
		var parent_select = generatePortForm('uplink_parent_select['+index+']', PARENT_PORTS.childNodes, port);
	...
	if(DEVICE_TYPE!='Host')
	{
		if(!document.getElementById('dodaj_uplink'))
		{
			var dodaj = document.createElement('input');
			dodaj.type = "button";
			dodaj.id = "dodaj_uplink";
			dodaj.value = "dodaj";
