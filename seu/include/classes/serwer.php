...
class Serwer extends Daddy
{
	...
	public function dodaj_do_magazynu($_device, $_sn, $_model, $_producent)
	{
		...
		if($this->device->dev_id != -1)
		{
			...
			$zapytanie = "INSERT INTO Serwer (device, sn, model, producent) 
			...
			{
				$this->loguj($this->device->dev_id, $this->device->lokalizacja, $user, 'Dodanie do magazynu', 'dodaj');
			}
		}
	}
