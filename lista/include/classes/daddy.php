...
class Daddy
{
	...
	{
		//$sql = mysql_connect('localhost', 'siec', 'WyGn2jEw0')or die(mysql_error());			
		$sql = mysql_connect('localhost', 'admin', 'WyGn2jEw0')or die(mysql_error());			
		if($this->sql != $sql)
		{
...
//		    exit();
	...
	//		print_r($opis);
		...
		case 'Host':
			...
			$opis['Numer_telefonu'] = $host['nr_tel'];
			$opis['Numer_telefonu_2'] = $host['nr_tel2'];
	...
	public function getSubnets($vlan)
	{
		...
		$zapytanie = "SELECT * FROM Podsiec WHERE vlan='$vlan' ORDER BY address";
		$wynik = mysql_query($zapytanie);
...
//		print_r($avaible_ports);
	...
	{
		$zapytanie = "SELECT Device.dev_id, Device.mac, Device.device_type  FROM Device, Lokalizacja WHERE Lokalizacja.Osiedle='MAGAZYN' AND Lokalizacja.id=Device.lokalizacja";
